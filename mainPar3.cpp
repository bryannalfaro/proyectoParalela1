...
NextPos getNextPosition(Square square1, float deltaTime)
{
    ...
    return {
        angleI,
        square1.initialX + square1.radius * cos(angleI),
        square1.initialY + square1.radius * sin(angleI),
    };
}
...
#pragma omp parallel for num_threads(numThreads) schedule(dynamic, 1)
            for (int j = 0; j < numberSquares; j++)
            {
                ...
                if (i != j)
                {
                    ...
                    int posIx = nextPosI.x;
                    int posIy = nextPosI.y;
                    ...
                    int otherPosIx = otherNextPosI.x;
                    int otherPosIy = otherNextPosI.y;
